...
// Include other necessary headers for your matrix-based SSSP implementation

// Placeholder for matrix-based shortest path approach:
std::vector<matrixvalue> run_matrix_based(const std::vector<std::vector<matrixvalue>> &adj, int source) {
    int n = static_cast<int>(adj.size());
    std::vector<matrixvalue> dist(n, empty);
    dist[source] = 0;
    // ... implement matrix-vector multiplication based SSSP logic ...
    return dist;
}
...
size_t calculate_memory_usage(const std::vector<std::vector<matrixvalue>> &adj) {
    size_t total = sizeof(adj); // Size of the outer vector object
...
int main(int argc, char* argv[]) {
    ...
    auto dist = run_matrix_based(adjacency, 0);
    auto end = std::chrono::steady_clock::now();
    std::chrono::duration<double> elapsed = end - start;
    std::cout << "Matrix-based SSSP on " << n << " nodes (p=" << p 
